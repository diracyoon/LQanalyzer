...
void Jet_Selection_Test_0::ExecuteEvents() throw(LQError)
{
  ...
  //jet match
  //for(Int_t i=0; i<4; i++){ target_jet[permutation_truth[i]] = kTRUE; }

  //count b tagged jets in target jets
  Int_t n_bjet_target = 0;
  for(Int_t i=0; i<n_jet_soft; i++){ if(chk_btag[i]==kTRUE && target_jet[i]==kTRUE) n_bjet_target++; }
  ...
  if(permutation_truth[0]==permutation_fitter[0] && permutation_truth[1]==permutation_fitter[1])
    {
      if((permutation_truth[2]==permutation_fitter[2]&&permutation_truth[3]==permutation_fitter[3])||(permutation_truth[2]==permutation_fitter[3]&&permutation_truth[3]==permutation_fitter[2])) jet_permutation_match = 1;
    }
